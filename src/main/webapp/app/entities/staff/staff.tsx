  ...
  return (
    <div>
      <h2 id="staff-heading" data-cy="StaffHeading">
        <Translate contentKey="serverTluContactApp.staff.home.title">Staff</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="serverTluContactApp.staff.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/staff/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="serverTluContactApp.staff.home.createLabel">Create new Staff</Translate>
          </Link>
        </div>
      </h2>
        ...
        {staffList && staffList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="serverTluContactApp.staff.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('education')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('education')} />
                </th>
                ...
                <th className="hand" onClick={sort('avatarUrl')}>
                  <Translate contentKey="serverTluContactApp.staff.avatarUrl">Avatar Url</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('avatarUrl')} />
                </th>
                <th>
                  <Translate contentKey="serverTluContactApp.staff.user">User</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="serverTluContactApp.staff.unit">Unit</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {staffList.map((staff, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{staff.email}</td>
                  <td>{staff.avatarUrl}</td>
                  <td>{staff.user ? staff.user.id : ''}</td>
                  <td>{staff.unit ? <Link to={`/unit/${staff.unit.id}`}>{staff.unit.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/staff/${staff.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/staff/${staff.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="serverTluContactApp.staff.home.notFound">No Staff found</Translate>
            </div>
          )
        )}
      </div>
